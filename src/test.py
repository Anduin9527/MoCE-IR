...
from data.custom_dataset import CustomTestDataset
...
def calc_ssim(img1, img2):
    return structural_similarity(
        img1, img2, channel_axis=2, gaussian_weights=True, data_range=1.0, full=False
    )
...
def run_test(opts, net, dataset, factor=8):
    ...
    if opts.save_results:
        pathlib.Path(
            os.path.join(
                os.getcwd(), f"results/{opts.checkpoint_id}/{opts.benchmarks[0]}"
            )
    ...
    with torch.no_grad():
        for [clean_name, de_id], degrad_patch, clean_patch in tqdm(testloader):
            ...
            assert restored.shape == clean_patch.shape, (
                "Restored and clean patch shape mismatch."
            )
...
def main(opt):
    ...
    for de in opt.benchmarks:
        ...
        if "CDD11" in opt.trainset:
            ...
            # For custom dataset, CustomTestDataset handles loading based on args.benchmarks internally or we pass it
            # But the loop iterator 'de' is one benchmark.
            # CustomTestDataset logic: it scans root_dir.
            # We should probably pass 'benchmarks=[de]' to it so it only loads that specific one for this iteration.
            # detailed logic: CustomTestDataset init calls _scan_dataset which filters by args.benchmarks
            dataset = CustomTestDataset(ind_opt, split="test")
        else:
